...
final class AuthManager: ObservableObject {
    ...
    func linkKingSoopers(presentationAnchor: ASPresentationAnchor) async throws {
        ...
        guard var components = URLComponents(string: authorizeResponse.authUrl) else {
            throw APIError.serverError(message: "Invalid auth URL from server")
        }
        let jwt = accessToken ?? ""
        var queryItems = components.queryItems ?? []
        queryItems.append(URLQueryItem(name: "state", value: jwt))
        components.queryItems = queryItems

        guard let krogerAuthURL = components.url else {
